...
class McarTests():
    ...
    def mcar_t_tests(self):
        ...
        mcar_matrix_value = pd.DataFrame(data=np.zeros(shape=(dataset.shape[1], dataset.shape[1])),
                                   columns=vars, index=vars)
        ...
        mcar_matrix_value = mcar_matrix[mcar_matrix.notnull()] 
        mcar_matrix = mcar_matrix[mcar_matrix.notnull()] > 0.05

        return mcar_matrix_value, mcar_matrix
