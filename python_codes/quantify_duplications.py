...
savedata_path = "/Volumes/MacintoshHD2/Users/haroonr/Detailed_datasets/IIIT_occupancy/july2018_occupancy_duplication/R_data/"
...
#%%
# saving data for R plotting
bldg = 'LB'
save_hourwise_dataframe(path, bldg, months_all, savedata_path,filename = "all.csv" )
save_hourwise_dataframe(path, bldg, months_break, savedata_path, filename = "summer.csv" )
save_hourwise_dataframe(path, bldg, months_semester, savedata_path,filename = "semester.csv" )
...
def compute_hourwise_count(path, bldg, months, savepath,filename):
  ...
  #return df_res
#%%
def save_hourwise_dataframe(path, bldg, months, savepath,filename):
  ''' In this I save data for R plotting system''' 
  month_data = []
  for i in range(0,len(months)):
    df = pd.read_csv(path + bldg + "/" + months[i])
    df.index = pd.to_datetime(df.timestamp)
  ...
  for i in range(0,24):
    dic[i] = list(df_groups.get_group(i).difference) # difference is column name
  df_res = pd.DataFrame.from_dict(dic,orient="index").T
  df_res.to_csv(savepath + bldg + "_" + filename, index = False)
  #plt.savefig(savepath + bldg + "_" + filename)
  #plt.close()
  #return df_res
