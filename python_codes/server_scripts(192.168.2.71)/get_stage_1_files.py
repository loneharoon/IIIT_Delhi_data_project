...
monthlist = ['data_2018_junejuly.json']
...
def  compute_stage_1_parallel(monthname):
    ...
    savepath =  monthname.split('.')[0]+'.csv'
    writefile = open(savepath,'w')
    csvfilewriter = csv.writer(writefile,delimiter=',')
    csvfilewriter.writerow(['trap_AP','trap_clientIP','trap_clientMAC','session_start','session_end'])
    with open(fl) as infile:
        for line in infile:
            ...
            trap_clientIP = packet['cldcClientIPAddress'] 
            trap_clientMAC = packet['cldcClientMacAddress']
